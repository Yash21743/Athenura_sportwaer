...
const ProductFilter = ({
  ...
  const pillInactive = { ...pillBase, borderColor: 'rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.7)' };
  const pillActive = { ...pillBase, borderColor: '#fff', background: '#fff', color: '#000', fontWeight: 600 };
  const pillCategory = (active) => active
    ? { ...pillBase, borderColor: '#FF3B30', background: '#FF3B30', color: '#fff', fontWeight: 600 }
    : { ...pillBase, borderColor: 'rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.7)' };
  ...
  return (
    <div ref={containerRef} style={{ marginBottom: '12px', fontFamily: 'Poppins, sans-serif' }}>
      ...
      <div style={{ position: 'relative', marginBottom: '14px' }}>
        <div style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)', color: 'rgba(255,255,255,0.3)', display: 'flex' }}>
          <Search size={15} />
        </div>
        <input
          type="text"
          ...
          onFocus={(e) => { e.target.style.borderColor = '#fff'; e.target.style.background = 'rgba(255,255,255,0.08)'; }}
          onBlur={(e) => { e.target.style.borderColor = 'rgba(255,255,255,0.1)'; e.target.style.background = 'rgba(255,255,255,0.05)'; }}
        />
        {searchTerm && (
          <button
            onClick={() => onSearchChange('')}
            style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)', cursor: 'pointer', display: 'flex', padding: '4px' }}
          >
            <X size={14} />
          </button>
        )}
      ...
      <div style={{ position: 'relative', marginBottom: '10px' }}>
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                onMouseEnter={(e) => { if(!isActive) { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.4)'; e.currentTarget.style.color = '#fff'; } }}
                onMouseLeave={(e) => { if(!isActive) { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.15)'; e.currentTarget.style.color = 'rgba(255,255,255,0.7)'; } }}
              >
                <span>{cat === 'All' ? 'All Categories' : cat}</span>
                {isActive && cat !== 'All'
                  ? <X size={13} style={{ opacity: 0.9 }} />
                  : <span style={{ fontSize: '11px', fontWeight: 600, padding: '1px 6px', borderRadius: '999px', background: isActive ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.1)', color: isActive ? '#fff' : 'rgba(255,255,255,0.6)' }}>{count}</span>
                }
              </button>
            );
          })}
        ...
        {/* right fade */}
        <div style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: '40px', background: 'linear-gradient(to right, transparent, #000)', pointerEvents: 'none' }} />
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
        ...
        <div style={{ position: 'relative' }}>
          <button 
            ...
            onMouseEnter={(e) => { if(priceRange === maxPriceLimit) { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.4)'; e.currentTarget.style.color = '#fff'; } }}
            onMouseLeave={(e) => { if(priceRange === maxPriceLimit) { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.15)'; e.currentTarget.style.color = 'rgba(255,255,255,0.7)'; } }}
          >
          ...
          <DropPanel open={openPanel === 'price'}>
            <div style={{ width: '260px', padding: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px' }}>
                <span style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Max Price</span>
        ...
        <div style={{ position: 'relative' }}>
          <button 
            ...
            onMouseEnter={(e) => { if(selectedSizes.length === 0) { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.4)'; e.currentTarget.style.color = '#fff'; } }}
            onMouseLeave={(e) => { if(selectedSizes.length === 0) { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.15)'; e.currentTarget.style.color = 'rgba(255,255,255,0.7)'; } }}
          >
          ...
          <DropPanel open={openPanel === 'size'}>
            <div style={{ width: '220px', padding: '16px' }}>
              <p style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '12px' }}>Select Sizes</p>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px' }}>
                {SIZES.map((sz) => {
                  const isSel = selectedSizes.includes(sz);
                  return (
                    <button key={sz} onClick={() => handleSizeToggle(sz)} style={{ height: '38px', borderRadius: '10px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', border: '1px solid', transition: 'all 0.15s', background: isSel ? '#FF3B30' : 'rgba(255,255,255,0.04)', borderColor: isSel ? '#FF3B30' : 'rgba(255,255,255,0.1)', color: isSel ? '#fff' : 'rgba(255,255,255,0.5)' }}>
                      {sz}
                    </button>
                  );
                })}
        ...
        <div style={{ position: 'relative' }}>
          <button 
            ...
            onMouseEnter={(e) => { if(selectedStockStatuses.length === 0) { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.4)'; e.currentTarget.style.color = '#fff'; } }}
            onMouseLeave={(e) => { if(selectedStockStatuses.length === 0) { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.15)'; e.currentTarget.style.color = 'rgba(255,255,255,0.7)'; } }}
          >
          ...
          <DropPanel open={openPanel === 'stock'}>
            <div style={{ width: '200px', padding: '16px' }}>
              <p style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '10px' }}>Availability</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                {STOCK_STATUSES.map((status) => {
                  const isSel = selectedStockStatuses.includes(status);
                  const dot = status === 'In Stock' ? '#4ade80' : status === 'Limited Stock' ? '#fbbf24' : '#f87171';
                  return (
                    <button key={status} onClick={() => handleStockToggle(status)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 12px', borderRadius: '10px', cursor: 'pointer', border: 'none', background: isSel ? 'rgba(255,59,48,0.1)' : 'transparent', color: isSel ? '#fff' : 'rgba(255,255,255,0.5)', fontSize: '12px', fontWeight: 500, textAlign: 'left', transition: 'all 0.15s' }}>
                      <div style={{ width: '16px', height: '16px', borderRadius: '5px', border: `1px solid ${isSel ? '#FF3B30' : 'rgba(255,255,255,0.2)'}`, background: isSel ? '#FF3B30' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <AnimatePresence>
          {selectedSizes.map((sz) => (
            <motion.button key={sz} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }}
              ...
              style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', height: '28px', padding: '0 10px', borderRadius: '999px', fontSize: '11px', fontWeight: 700, background: 'rgba(255,59,48,0.1)', border: '1px solid rgba(255,59,48,0.25)', color: '#FF3B30', cursor: 'pointer' }}
            >
              {sz} <X size={10} />
            </motion.button>
          ))}
        ...
        <AnimatePresence>
          {isAnyFilterActive && (
            <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', height: '34px', padding: '0 14px', borderRadius: '999px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', marginLeft: 'auto', border: '1px solid rgba(255,59,48,0.2)', background: 'rgba(255,59,48,0.05)', color: 'rgba(255,59,48,0.8)', transition: 'all 0.18s' }}
            >
              <X size={12} /> Clear all
            </motion.button>
          )}
