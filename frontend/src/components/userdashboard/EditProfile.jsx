      ...
      {title && (
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", borderBottom: "1px solid rgba(10, 127, 110, 0.18)", paddingBottom: "14px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            {icon && (
              <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: "rgba(10,127,110,0.08)", color: "#0A7F6E", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                {icon}
              </div>
            )}
            <div>
              <h3 style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 800, fontSize: "15.5px", color: "#0f172a", letterSpacing: "0.2px", margin: 0 }}>{title}</h3>
...
const EditProfile = ({ profile, setProfile }) => {
  ...
  // ✅ PUT /api/users/me instead of only local state
  ...
  return (
    <div style={{ animation: "csw-fadein 0.45s ease both", fontFamily: "'Poppins', sans-serif" }}>
      <div className="csw-main-grid" style={{ gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
        <Card title="Profile Details" sub="Configure credentials" icon={<User size={18} />} accent="#0A7F6E">
          <form onSubmit={handleProfileSubmit} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            <div>
              ...
              <div style={{ position: "relative" }}>
                ...
                <input type="text" value={name} onChange={e => setName(e.target.value)} required
                  style={{ width: "100%", background: "#ffffff", border: "1px solid #cbd5e1", padding: "10px 14px 10px 40px", borderRadius: "10px", color: "#0f172a", fontSize: "13px", outline: "none", fontFamily: "'Poppins', sans-serif", boxSizing: "border-box" }}
                />
              </div>
            </div>
            ...
            <button type="submit" disabled={savingProfile} className="ep-submit-btn"
              style={{ marginTop: "8px", padding: "11px 24px", background: "linear-gradient(135deg, #0A7F6E, #0d9488)", border: "none", borderRadius: "10px", color: "#fff", fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.6px", cursor: savingProfile ? "not-allowed" : "pointer", fontFamily: "'Poppins', sans-serif", transition: "all 0.22s ease", boxShadow: "0 4px 12px rgba(10,127,110,0.15)", opacity: savingProfile ? 0.7 : 1 }}
            >
        ...
        <Card title="Change Password" sub="Secure login details" icon={<Lock size={18} />} accent="#0A7F6E">
          <form onSubmit={handlePasswordSubmit} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <button type="submit" disabled={savingPassword} className="ep-submit-btn"
              style={{ marginTop: "8px", padding: "11px 24px", background: "linear-gradient(135deg, #0A7F6E, #0d9488)", border: "none", borderRadius: "10px", color: "#fff", fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.6px", cursor: savingPassword ? "not-allowed" : "pointer", fontFamily: "'Poppins', sans-serif", transition: "all 0.22s ease", boxShadow: "0 4px 12px rgba(10,127,110,0.15)", opacity: savingPassword ? 0.7 : 1 }}
            >
      ...
      <style>{`
        .ep-submit-btn { width: 100%; }
        @media (min-width: 640px) {
          .ep-submit-btn { width: fit-content !important; padding: 11px 36px !important; display: block !important; margin-left: auto !important; margin-right: auto !important; }
        }
      `}</style>
    </div>
  );
