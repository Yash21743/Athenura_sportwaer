...
//             muted
//             playsInline
//             style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: '85% 35%', opacity: 0.6, transform: 'scale(1.2)' }}
//           >
...
//             style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.14em', fontSize: 'clamp(12px, 2vw, 18px)', color: 'rgba(255,255,255,0.9)', marginBottom: '12px' }}
//           >
//             Performance Wear for Junior Athletes
//           </motion.p>
...
//               <div style={{ position: 'absolute', inset: 0, border: '2px solid transparent', borderTopColor: '#ea580c', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
//             </div>
...
//                   onMouseEnter={(e) => { e.currentTarget.style.background = '#0A7F6E'; e.currentTarget.style.color = '#ffffff'; e.currentTarget.style.borderColor = '#0A7F6E'; }}
...
const KidsProducts = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        // "Kids" section — Unisex products automatically included by backend
        const response = await API.get('/products?gender=Kids');
        const list = response.data?.data || response.data;
        if (
          list &&
          Array.isArray(list) &&
