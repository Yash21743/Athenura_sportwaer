...
const Products = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        // "Men" section — Unisex products automatically included by backend
        const response = await API.get('/products?gender=Men');
        const list = response.data?.data || response.data;
        if (
          list &&
          Array.isArray(list) &&
          ...
          // ✅ FIX: normalize backend shape → what ProductCard expects.
          // Backend sends images as [{ url, publicId }] objects, sizes/colors
          // as availableSizes/availableColors, and category as a populated
          // object — none of which ProductCard understands directly.
